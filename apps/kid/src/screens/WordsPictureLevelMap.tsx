...
const PICTURE_LEVEL_LABELS: Record<number, { fr: string; en: string }> = {
  1: { fr: 'Objet', en: 'Object' },
  2: { fr: 'Adjectif', en: 'Adjective' },
  3: { fr: 'Action', en: 'Action' },
  4: { fr: 'Phrase 2 mots', en: '2-word phrase' },
};

export function WordsPictureLevelMap({
  onLevel,
  onHome,
  onBack,
}: {
  ...
  // Picture sub-mode only — Words bundles all 4 sub-modes together.
  const picture = useMemo(
    () => (bundle ? bundle.questions.filter((q) => q.sub_mode === 'picture') : []),
    [bundle],
  );
  ...
  // Words/Picture is language-DEPENDENT (product §7.3): progress tracked per language.
  const levels = profile?.progress_by_module_per_language.words_picture[lang].levels ?? [];
  const isComplete = (lvl: number) =>
    levelComplete(levels, lvl, unitsForLevel(lessonsForLevel(picture, lvl)));
  const labelFor = (lvl: number) => PICTURE_LEVEL_LABELS[lvl]?.[lang] ?? `${t('level')} ${lvl}`;
