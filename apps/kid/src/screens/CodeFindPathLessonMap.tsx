...
// Code/Find-the-path lesson map: a level's lessons (gated in order) + a revision if
// ≥ 2 lessons. Mirrors NumbersLessonMap, but reads from the localStorage-backed
// per-sub-mode track because Code's canonical sync track lumps both sub-modes.
export function CodeFindPathLessonMap({
  level,
  onUnit,
  onHome,
  onBack,
  ...
  const units = useMemo(
    () => unitsForLevel(lessonsForLevel(findPath, level)),
    [findPath, level],
  );
