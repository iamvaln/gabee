// The admin mascot — coral body (`#FF8A6B`), everything else per the standard mascot
// (admin spec §4.6). Ported from the admin design handoff.

export type AdminBeeExpression = 'idle' | 'focus' | 'correct' | 'celebrate';

export function AdminBee({
  size = 96,
  expression = 'idle',
  wings = true,
  style = {},
  ...
  size?: number;
  expression?: AdminBeeExpression;
  wings?: boolean;
  style?: React.CSSProperties;
}) {
  const body = '#FF8A6B';
  const dot = (cx: number) => <circle cx={cx} cy="11" r="3" fill="#2BD4E6" />;
  const up = (cx: number) => <circle cx={cx} cy="8" r="3" fill="#2BD4E6" />;
...
function BeeGlyph({ size = 30 }: { size?: number }) {
  return (
  ...
  return (
    <span className="bee-logo" style={{ display: 'inline-flex', alignItems: 'center', gap: 1 }}>
      ...
      <span
        ...
        style={{ fontWeight: 900, fontSize: size * 0.74, letterSpacing: '-0.03em', color: 'var(--ink)' }}
      >
        abee
      </span>
    </span>
