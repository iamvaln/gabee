...
// Password credentials live in a SEPARATE, history-capable table (never on the account),
// so we can retain password history if needed. Auth is our own (not Supabase Auth).
ParentCredential {
  id: uuid
  parent_id: uuid
  hash: string            // Node scrypt
  salt: string
  algorithm: string       // 'scrypt'
  created_at
  retired_at: timestamp?  // null = active; a password change retires the row + inserts a new active one
}
...
The landing page, parent dashboard, and admin back office are **path-segment surfaces within a single Next.js app** (App Router — `/` landing, `/parent`, `/admin`) that also hosts the API (route handlers) the kid PWA calls — see §11. They share auth (own email/password), types (`packages/types`), and the database in-process. Each surface is served from its own subdomain of the one domain: `gabee.app` (landing), `parents.gabee.app`, `admin.gabee.app`, `api.gabee.app`.
...
```
  ...
  │  Vite + React      │  REST/JSON  │  • landing (apex `/`)                              │
  │  offline · Dexie   │             │  • /parent dashboard    • /admin back office       │
  └────────────────────┘             │  • app/api/* route handlers                        │
                                      │  • Own auth (scrypt+jose) · Claude (LLM)           │
                                      └─────────────────────────┬──────────────────────────┘
                                                                │ Prisma
                                                      ┌─────────▼──────────────┐
                                                      │  PostgreSQL (Supabase)  │
...
| Layer | Tech |
|---|---|
| Framework | **Next.js (App Router)** — role-gated path segments: `/` landing · `/parent` · `/admin`; API in `app/api/*`; `Host`-based `proxy` routing |
| Styling | Tailwind |
...
| Auth | **Own email/password** — Node `scrypt` hashing + `jose` JWT sessions (httpOnly cookie for web, bearer for the kid app); credentials in a separate, history-capable table. **Not** Supabase Auth (avoids vendor lock-in) |
...
| i18n | **i18next** |
| API access | **CORS + the parent's JWT (bearer)** — our own `jose`-signed token — to `api.gabee.app` (kid device pairs once, §7.2) |
...
**Hosting**
- **Preprod/staging**: **Vercel** (Next app is Vercel-native; the kid PWA deploys there as static too) + Supabase (managed **Postgres** only; auth is our own) + Mailgun. Local dev uses a local Postgres.
...
All of `gabee.app`, `parents.gabee.app`, `admin.gabee.app`, and `api.gabee.app` point at the **same `apps/web` deployment** — a subdomain is an address, not a separate codebase. The Next 16 **`proxy`** (`src/proxy.ts`, the renamed `middleware`) routes by `Host`. Surfaces are **path segments** (`/parent`, `/admin`), not URL-invisible route groups — so the proxy can target them and they work locally without subdomains:

- `gabee.app` → the landing (apex, `app/page.tsx`)
- `parents.gabee.app` → `/parent` (requires a parent session)
- `admin.gabee.app` → `/admin` (**requires an admin session**; optionally IP-allowlisted)
...
- **Architecture & stack** (full detail in §11): TypeScript end-to-end · monorepo pnpm + Turborepo · **one Next.js (App Router) app** = landing + parent + admin + API route handlers — **no separate backend service** · validation **Zod** · **Prisma + Postgres** (Supabase-hostable; local Postgres in dev) · **own email/password auth** (Node `scrypt` + `jose` JWT; credentials in a separate, history-capable table — not Supabase Auth) · **Claude API** (provider-abstracted) · web i18n **next-intl** · kid app is a **Vite + React PWA** (Dexie · vite-plugin-pwa · Zustand · TanStack Query · Motion · Howler · **i18next**) that calls the API via **CORS + the parent's JWT** (our own `jose`-signed bearer)
