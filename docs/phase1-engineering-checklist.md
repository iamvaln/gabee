...
- ✅ **Auth** — own email/password (Node `scrypt` hashing + `jose` JWT; credentials in a separate, history-capable table) — **not** Supabase Auth
...
- [ ] App scaffolds: `apps/web` (Next.js — landing (apex) + `/parent` + `/admin` path segments + `app/api/*`, routed by `Host`-based `proxy`), `apps/kid` (Vite PWA); package scaffolds: `packages/types`, `packages/db`
- [ ] (`/admin` surface + `packages/ui` deferred to Phase 2 — Phase 1 ships the landing (apex) + `/parent` + the API only)
...
- [ ] **CORS** on the `api.gabee.app` handlers for the `kids.gabee.app` origin; **bearer-JWT** (the parent's own `jose`-signed token) on all kid-facing endpoints
- [ ] **Auth**: own email/password — Node `scrypt` hashing + `jose` JWT sessions (httpOnly cookie for web, bearer for the kid app); credentials in a separate, history-capable table (parent signup + login)
...
## 5. Parent surface (`/parent` routes in `apps/web`) — Phase 1 minimal

> Phase 1 = just enough to classify sessions and read adherence. The rich dashboard is Phase 2.

- [ ] Next `/parent` routes at `parents.gabee.app`, parent login (own email/password)
- [ ] Minimal landing (apex) at `gabee.app` with a parent sign-up entry (full marketing copy can come later)
...
TypeScript everywhere · monorepo pnpm + Turborepo · **one Next.js (App Router) app** = landing + parent + admin + API route handlers (no separate backend) · **Zod** contracts · **Prisma + Postgres** (Supabase-hostable; local Postgres in dev) · **own email/password auth** (Node `scrypt` + `jose` JWT) · web i18n **next-intl** · kid app **Vite + React PWA** (Dexie · vite-plugin-pwa · Zustand · TanStack Query · Motion · Howler · **i18next**) calling the API via CORS + the parent's JWT · **Mailgun** email · single domain **`gabee.app`** (`parents.` / `admin.` / `api.` / `kids.`) · preprod **Vercel**, production **VPS**.
