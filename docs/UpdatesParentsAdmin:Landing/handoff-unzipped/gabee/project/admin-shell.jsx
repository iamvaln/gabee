...
function AIcon({ name, size = 18 }) {
  ...
  switch (name) {
    ...
    case 'heart': return <svg {...s}><path d="M12 20s-7-4.4-7-10a4 4 0 0 1 7-2 4 4 0 0 1 7 2c0 5.6-7 10-7 10Z"/></svg>;
    case 'logout': return <svg {...s}><path d="M15 4h3a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-3"/><path d="M10 12H3M6 8l-3 4 3 4"/></svg>;
    default: return null;
  }
}
...
const NAV = [
  ...
  { id: 'msg-health', icon: 'heart', label: { fr: 'Santé Messages', en: 'Messages health' } },
...
function Sidebar({ route, lang, role, setRole, onNav, onLanding, onSignOut }) {
  ...
  return (
    <aside className="side">
      ...
      <button className="side-landing" onClick={onLanding}>
        <span className="ni-icon"><AIcon name="external" /></span>
        <span>{lang === 'fr' ? 'Voir le site public' : 'View public site'}</span>
        <span className="sl-host">gabee.app</span>
      </button>
      <div className="side-foot">
        <div className="acct">
          ...
          {onSignOut && (
            <button className="acct-out" onClick={onSignOut} aria-label={lang === 'fr' ? 'Se déconnecter' : 'Sign out'} title={lang === 'fr' ? 'Se déconnecter' : 'Sign out'}>
