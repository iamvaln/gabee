...
function PlanEditor({ lang, moduleId = 'numbers', state = 'draft', onOpenPool }) {
  const L = lang === 'fr';
  const p = PLANS[moduleId] || PLAN_DRAFT;
  const mod = MOD.find(m => m.id === moduleId) || MOD[0];
  ...
  return (
    <div className="page">
      <PageHead title={`${mod.name[lang]} · ${L?'Niveau':'Level'} ${crumbLevel}`}
        ...
        {state !== 'disabled' && <>
          <span className="author-mod"><ModuleDot id={moduleId} size={10} />{mod.name[lang]}</span>
      ...
      {state === 'disabled' && (
        <div className="disabled-note">
          ...
          <div>{L ? `Le niveau précédent (${p.prevName.fr}) n’a pas encore de plan accepté. La continuité est garantie par construction — ` : `The previous level (${p.prevName.en}) has no accepted plan yet. Continuity is enforced by construction — `}
            <a>{L ? `planifiez d’abord le niveau ${p.level-1}.` : `plan level ${p.level-1} first.`}</a></div>
        </div>
      )}
      {state === 'error' && (
      ...
      <fieldset disabled={state==='disabled'} style={{ border:0, padding:0, margin:0, opacity: state==='disabled'?0.5:1 }}>
        ...
        <details className="prev-context">
          ...
          <div className="pc-body">
            {p.prev.map((lv, i) => (
              <div key={i} className="pc-lvl">
                <div className="pc-lvl-h">{lv.h[lang]}</div>
                <ul>{lv.items.map((it, j) => <li key={j}>{it[lang]}</li>)}</ul>
              </div>
            ))}
          </div>
        </details>
...
function QuestionPool({ lang, moduleId = 'numbers', state = 'review', onModal }) {
  const L = lang === 'fr';
  const target = POOL_TARGET;
  const mod = MOD.find(m => m.id === moduleId) || MOD[0];
  const p = PLANS[moduleId] || PLAN_DRAFT;
  const pool = POOLS[moduleId] || CANDIDATES;
  const [progress, setProgress] = React.useState(state === 'generating' ? 0 : 30);
  const [cards, setCards] = React.useState(state === 'generating' ? [] : pool);
  const [ratings, setRatings] = React.useState(() => Object.fromEntries(pool.map(c => [c.id, { fr: c.rFr, en: c.rEn }])));
  ...
  React.useEffect(() => {
    ...
    const id = setInterval(() => {
      n += 1;
      setProgress(n);
      if (n <= pool.length) setCards(pool.slice(0, n));
  ...
  return (
    <div className="page">
      <PageHead title={`${L?'Pool — ':'Pool — '}${mod.name[lang]} · ${L?'Niveau':'Level'} ${p.level}`}
      ...
      <div className="card card-pad pool-head" style={{ display: state==='empty'||state==='disabled' ? 'none' : 'grid' }}>
        <div>
          ...
          <div className="wrap-actions">
            {p.objectives.slice(0,3).map((o,i)=>(
      ...
      {(state==='review' || state==='generating') && (
        <div className="cand-grid mt16">
          {cards.map((c, idx) => {
            ...
            return (
              <div key={c.id} className={'cand' + (idx===pool.length-1 && c.status==='rejected'?' rejected':'')}>
...
function GenModal({ lang, moduleId = 'numbers', onClose }) {
  const L = lang === 'fr';
  const mod = MOD.find(m => m.id === moduleId) || MOD[0];
  const p = PLANS[moduleId] || PLAN_DRAFT;
  return (
      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="banner offline" style={{ background:'var(--surface-2)', color:'var(--text-2)', margin:0 }}>
            ...
            <div><b style={{color:'var(--ink)'}}>{`${mod.name[lang]} · ${L?'Niveau':'Level'} ${p.level}`}</b><span className="b-sub"> · {L?'le plan accepté est utilisé comme base':'the accepted plan is used as the base'}</span></div>
          </div>
          <div>
