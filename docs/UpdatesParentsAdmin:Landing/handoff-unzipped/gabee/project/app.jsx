...
function GabeeApp() {
  ...
  const seededRouteRaw = parseHash();
  const seededReader = seededRouteRaw && seededRouteRaw.name === 'reader';
  const seededBandeau = seededRouteRaw && seededRouteRaw.name === 'bandeau';
  const seededRoute = (seededReader || seededBandeau) ? { name: 'home' } : seededRouteRaw;
  const seededProfile = seededRouteRaw ? { ...PROFILES[0], name: 'Léo' } : null;
  ...
  // Parent → kid messages: pending queue + reader overlay
  const [pendingMsgs, setPendingMsgs] = React.useState(KID_MESSAGES);
  const [readerMsg, setReaderMsg] = React.useState(seededReader ? KID_MESSAGES[0] : null);
  const finishReader = () => {
    setPendingMsgs(q => q.filter(x => !readerMsg || x.id !== readerMsg.id));
    setReaderMsg(null);
  };
  ...
  const sceneJumps = [
    ...
    { id: 'msg-bandeau', label: 'Parent message — bandeau', go: () => { if (!profile) setProfile(PROFILES[0]); setReaderMsg(null); setPendingMsgs(KID_MESSAGES); nav({ name: 'home' }); } },
    { id: 'msg-reader', label: 'Parent message — reader', go: () => { if (!profile) setProfile(PROFILES[0]); setPendingMsgs(KID_MESSAGES); setReaderMsg(KID_MESSAGES[0]); nav({ name: 'home' }); } },
    { id: 'settings', label: 'Kid settings', go: () => { if (!profile) setProfile(PROFILES[0]); nav({ name: 'settings' }); } }
  ];

  const isEmbedded = seededRoute !== null;

  const betweenLessons = ['home', 'summary', 'celebrate', 'daycap'].includes(route.name);
  const bandeauMsg = pendingMsgs[0];

  return (
    <div className={stageClass}>
      <div className="kid-frame">
        {renderScreen()}
        {profile && betweenLessons && bandeauMsg && !readerMsg && (
          <MessageBandeau lang={lang} message={bandeauMsg} audio={audio} onTap={() => setReaderMsg(bandeauMsg)} />
        )}
        {readerMsg && <MessageReader lang={lang} message={readerMsg} onContinue={finishReader} />}
      </div>
