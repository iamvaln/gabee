...
The parent app exists so a parent **stays close to their child's learning**, in three rhythms:

1. **The daily ritual** — see what your child has been doing, classify whether they asked to play or you suggested, and leave them a word for their next session. This is the bond Gabee maintains between parent and child through the screen.
2. **The deep look** — on your own time, see how your kid is doing per module, where they're growing, where they're stuck. Per-kid detail.
3. **The feedback channel** — shape Gabee with ratings and comments on any item; the team uses these to improve the content.

Beyond these, account / family / device / notification management is in Settings.

**Architecture choices that follow:**
- **Bringing the parent into the kid's session.** The classification ritual and the messaging feature aren't just data collection — they're a small bridge between the parent and what the kid is doing on the other side of the screen. A child seeing *« Papa t'a laissé un mot »* mid-session is the whole point.
...
Top-bar navigation, **five destinations**:

```
[Ga-bee logo (mint)]   Home   Classification(N)   Kids   Messages(M)   Settings    [🔔]  [FR/EN]  [Avatar▾]
```
...
- **Messages** (§8) — list of messages sent to each kid, compose new at any time; the `(M)` badge counts messages still unread by the kid (the parent sees what's still pending).
- **Settings** (§10) — profile, password, family / co-parents, paired devices, notifications, account deletion.
- **Avatar menu** — Profile, Sign out.
- **Bell** — alternative entry to classification + recent admin replies on feedback.

At narrow widths, the top nav collapses into a bottom tab bar (Home · Classification · Kids · Messages · Settings) for thumb reach; at wide widths it stays as a horizontal top bar. Deep links from email always land directly on the target screen regardless of device.

**Feedback** (§11) is *contextual* — it appears inline as a "Rate this" affordance next to module / level / lesson / question items wherever the parent encounters them.
...
The daily moment that keeps the parent close to what their child is doing. Reached from: email link (primary entry), home card (5.1), top-nav menu (Classification), bell.

### 6.1 Purpose

Each day, Gabee asks the parent a small question about each of their kid's sessions: did they ask to play, or did you suggest it? Two minutes a day. The point is **participation** — the parent stays informed, stays close to the learning, and at the end of the flow can leave the child a word for the next session (see §8 Messages). Behind the scenes, the parent's answers feed a derived adherence metric used internally to understand how well the product is fitting families; but for the parent, this is a daily ritual, not a survey.
...
- **Three large buttons** (large enough for thumb taps):
  ...
  - **Not sure** (counts as unknown, removed from queue)
- **Skip for now** (small text link, keeps in queue).

After submission, the next session loads automatically. Progress bar at top ("3 / 8").

**On the last session**, the thank-you screen offers an optional **"Leave `<Name>` a word for next time"** action:

- Tap → opens the Compose Message modal (C1.1), pre-filled with `to_child_id = <current kid>`.
- Send → message goes to `<Name>`'s queue, shown at the end of their next lesson (see §8).
- Skip → return to home.

Mascot in celebrate expression. If the parent has multiple kids with completed classification today, the action lets them pick which kid (or all).

### 6.3 Entry points & deep-linking
...
**C1.1 · Compose message (inline)** — invoked from the end-of-flow thank-you screen; same behaviour as M2 in §8.2.
...
A standalone surface where the parent can write a short message to any of their kids, at any time. Messages reach the kid in the kid app — surfaced as a small bandeau at the end of a lesson, persistent until read.

### 8.1 Purpose

Three reasons messages exist:

1. **Closeness through the screen.** A short word from a parent lands in the kid's session — *« Bravo Ana, papa a vu que tu as fini ton niveau 3 ! »* — and the kid feels seen. Screen time stops being isolated.
2. **Participation as part of the ritual.** Beyond classification, the parent has another concrete way to engage with their child's learning journey — independent of any prompt.
3. **Always available.** Unlike the classification flow (anchored to yesterday's sessions), Messages can be sent at any moment — before school, before bed, mid-day — from the dedicated menu.

### 8.2 Flow

**M1 · Messages list** — main screen of the Messages section. Reached from the top-nav "Messages" menu.

Top: filter chips by kid (`All` / `<Kid1>` / `<Kid2>` / ...), default `All`.
Below: large primary button **+ New message**.
Below: chronological list (newest first) of every message ever sent, each row showing:

- Kid avatar + name (recipient)
- Sender label (if the parent has a co-parent — otherwise omitted)
- Truncated message preview (1 line)
- Status: `Unread` (mint dot), `Read` (timestamp like "Read 2h ago"), or `Deleted before read` (greyed)
- Tap a row → detail (M3).

**M2 · Compose message** — modal (full-screen on narrow widths, centred modal on wide).

- **To**: kid picker (avatars, one selected). Required.
- **Text field**: textarea, max **200 characters**, live counter.
- **From**: shows the parent's `display_name_for_kids` (defaults to first name, customisable in §11 Settings → Profile) — *« Le message sera signé "Papa". Changer ? »*.
- **Send** (primary, mint) · **Cancel** (ghost).
- On send: optimistic — message appears in M1 with `Unread` status, modal closes.

**M3 · Message detail** — reached by tapping a row in M1.

- Full message text in large readable style.
- Metadata: to whom, from whom (if multi-parent), sent at, read at (if read).
- If `Unread`: a **Delete this message** action (mint outline button) — removes it from the kid's queue. Confirm dialog: *« Le message n'a pas encore été lu par `<Name>`. Supprimer ? »*.
- If `Read`: no edit / delete (message has reached the kid; the bond is established and can't be retracted).

### 8.3 Limits & rules

- **Length**: 200-character cap (enough for a meaningful sentence or two; small enough to not overwhelm a 6-8 year-old reader).
- **No media** at MVP: text only. No images, no audio, no attachments.
- **No frequency cap** at MVP: parents self-regulate. If abuse appears in data, a cap can be added later.
- **No reply from the kid**: messaging is unilateral parent → kid (a child of 6-8 is not expected to type back; the kid's "reply" is showing up for the next session).
- **No moderation**: messages are between family members; the admin does not see message content. Only counts surface in admin analytics (see admin spec §11).
- **Co-parent**: each parent's messages are independent. The recipient kid sees each sender's name (per §11 Settings → Profile → `display_name_for_kids`).

### 8.4 Kid-side reception

When the kid is in a session and a parent has sent them a message:

- At the **end of the current lesson** (between lessons, never mid-question), a small bandeau appears at the bottom of the kid app: *« <DisplayName> t'a laissé un mot 💛 »*, with a soft "ding" cue (consistent with Gabee's existing audio identity for correct-answer feedback).
- The bandeau is **persistent until tapped**: if the kid ignores it and starts the next lesson, the bandeau reappears at the end of that one. It never disappears on its own.
- Tap → message text shown big and centred → **Continue** button (mint) → returns to the kid app exactly where they were.
- Once tapped, the message is marked `Read` (timestamp recorded) and the parent sees it as `Read` on next sync.
- If multiple unread messages exist for the same kid, oldest is shown first; the next bandeau appears at the end of the following lesson.

This is detailed on the kid-side in `gabee-ux-spec.md` (the receiving screen + bandeau component + sound cue).

### 8.5 Entry points

- **Primary**: top-nav **Messages** menu.
- **Secondary**: from the end of the Classification flow (§6.2 thank-you screen — *« Laisser un mot ? »*).
- **Secondary**: from the Kid detail screen (§7 K2 — *« + Laisser un mot à `<Name>` »* button).

All three open M2 (Compose) with the appropriate kid pre-selected; on send, the parent lands in M1 with the new message at the top.

### 8.6 Screens & states

| ID | Screen | States |
|---|---|---|
| **M1** | Messages list | `Default`, `Loading`, `Empty` (mascot idle, *« Aucun message pour l'instant. Écris-leur un mot ! »* + primary CTA), `Filter active`, `Offline` |
| **M2** | Compose message | `Default`, `Sending`, `Error`, `Over-limit` (counter red, send disabled) |
| **M3** | Message detail | `Unread` (delete affordance shown), `Read` (read-only), `Deleted` (no longer in list, but if linked directly, shows "Message removed before being read") |

---

## 9. Family & co-parents (Phase 1)
...
### 9.3 Family activity log (Phase 1)
...
### 10.4 Paired devices & the kid-app link (Phase 1)
...
### 11.1 Inline feedback affordance
...
### 12.4 Login & forgot password (Phase 1)
...
ParentAccount {
  ...
  first_name: string
  last_name: string
  display_name_for_kids: string          // how the kids see this parent (e.g. "Papa", "Maman", "Mamie"); defaults to first_name on creation, customisable in Settings → Profile
  country: string
  ui_language: 'fr' | 'en'
...
// Parent → kid messages (§8)
KidMessage {
  id: string
  from_parent_id: string                 // FK ParentAccount
  to_child_id: string                    // FK ChildProfile
  text: string                           // max 200 chars (enforced at write time)
  status: 'unread' | 'read' | 'deleted_by_sender'
  created_at: Date
  read_at?: Date                         // set when kid taps the bandeau (§8.4)
  deleted_at?: Date                      // set when sender deletes an unread message (§8.2 M3)
  // No edit after send. Messages are immutable text once persisted.
  // Once status='read', the message is locked (no delete possible).
}
...
FamilyActivityLog {
  ...
  payload: any                           // small JSON: session_id / feedback_id / device_id / message_id / etc.
  created_at: Date
}
