...
```
gabee/
├── apps/web/        # Next.js (App Router): landing (apex) + /parent + /admin + app/api/* — this IS the backend
...
Stack: **one Next.js app (App Router)** = landing + parent + admin + **API route handlers** — **no separate backend** · **Zod** (shared contracts) · **Prisma + Postgres** (Supabase-hostable; **local Postgres in dev**) · **own email/password auth** — Node `scrypt` hashing + **`jose`** JWT sessions; credentials in a separate, history-capable table (**not** Supabase Auth, so we're not vendor-locked) · **Claude API** (provider-abstracted) · web i18n **next-intl** · **Recharts** (charts) · **TanStack Table** (admin) · kid app **React + Vite + Tailwind PWA** — **Zustand** (local) + **TanStack Query** (server/sync) · **Dexie** (IndexedDB) · **vite-plugin-pwa** (Workbox under the hood) · **Motion** (`motion/react`) · **Howler.js** · **i18next** · **Mailgun** (email).

Kid app calls `api.gabee.app` via **CORS + the parent's JWT** (our own, `jose`-signed bearer). Domains: one registrable domain **`gabee.app`** (wildcard SSL) — `gabee.app` (landing) · `parents.` · `admin.` · `api.` (Next) · `kids.` (kid PWA). Hosting: preprod **Vercel** (Next + kid static); production a **VPS** (Next `output: 'standalone'` container behind nginx; kid app static). All four Next subdomains are the **same `apps/web` deployment**, routed by **`Host`-based `proxy`** (Next 16 file convention; formerly `middleware`): `api.` exposes only `/api/*` (+ CORS for `kids.`), `admin.` → `/admin` behind an admin-session gate, `parents.` → `/parent`, apex → the landing. Surfaces are **path segments** (`/parent`, `/admin`), not URL-invisible route groups, so they're targetable by the proxy and work locally without subdomains. Only the kid app is cross-origin; parent/admin call same-origin routes (no CORS).
...
- Web app (Next, `apps/web`): **API route handlers** for auth, profiles, versioned question bundles, event ingestion, progress sync, classification queue, daily email digest, data-view aggregates; plus a minimal landing (apex) and the `/parent` surface.
...
- Parent surface: the `/parent` routes — session classification + minimal per-child data view.
- ~60 seeded questions per module (FR + EN), inserted directly via seed scripts.

**Out (do NOT build):** the `/admin` surface + authoring UI, AI generation pipeline, full parent dashboard, voiceover, avatar recoloring, badges/streaks, levels 4-10. These are Phase 2/3.
...
2. **Web app + API core** — Next.js (App Router); API as **Route Handlers** in `app/api/*` with a `lib/server/*` services layer and **Zod** at every boundary; **own email/password auth** (Node `scrypt` hashing + `jose` JWT; cookie for web, bearer for the kid app; credentials in a separate history-capable table); profiles; Postgres via Prisma + migrations; **CORS for the `kids.` origin + bearer-JWT** on kid-facing endpoints; `Host`-based `proxy` routing.
...
8. **Parent surface** — the `/parent` routes (classification queue + minimal data view) + a minimal landing (apex); wire the email digest.
...
- **Auth is self-managed** — own email/password (Node `scrypt` hashing + `jose` JWT sessions), **not** Supabase Auth / `@supabase/ssr`. Supabase, if used at all, is only managed Postgres (dev runs on local Postgres).
- **Next 16 `middleware` → `proxy`** — host routing lives in `src/proxy.ts` exporting `proxy()`; the `middleware` file convention is deprecated.
...
| DB (managed Postgres, optional) | Supabase | https://supabase.com/docs |
| Auth — sessions | jose (JWT) | https://github.com/panva/jose |
| Auth — password hashing | Node `scrypt` (built-in) | https://nodejs.org/api/crypto.html |
