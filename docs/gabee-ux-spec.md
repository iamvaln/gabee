...
1. App opens → device not yet linked → "Ask a grown-up" screen → parent signs in (email+password, own auth) or creates account.
...
1. Email + password (own auth) → verify → create household → add first child profile (F2) → link the kid device.
