...
- **Companions**: `product-spec-v0.1.md` (the *what*), `gabee-ux-spec.md` (kid + parent UX), `gabee-design-spec.md` (tokens & brand — used as-is here; admin's own variants live in §4.6).
...
### 4.6 Admin visual identity

The admin uses the same Gabee tokens as the rest of the system (palette, typography, radii, spacing, motion — see `gabee-design-spec.md`) with **two admin-specific variations** spelled out here so this spec is self-contained.

**Wordmark — text-only "Ga-Bee".**
- Set in **Mulish 800 ink** (`#20242E`), with the dash: **Ga-Bee**.
- **No bee illustration in the wordmark.** Used in the top-bar logo position, the login screen, and any place the brand mark appears in admin chrome.
- Rationale: the back office is an internal authoring tool. A tight, text-only wordmark fits the context.

**Mascot — coral body.**
- When the bee appears as an illustration in admin surfaces (loading states, empty states, success toasts, decoration, the dashboard hero), use **coral `#FF8A6B`** for the body.
- Everything else of the bee stays identical to the standard mascot in `gabee-design-spec.md §2`: **cyan eyes** (`#2BD4E6`), **ink visor** (`#20242E`), **light-cyan wings** (`#BBEAF2`), **dark stripes** (ink), **antennae** (ink with cyan ball tips).
- The visor **expression system** (`gabee-design-spec.md §3` — idle / correct / celebrate / encourage / focus) applies wherever it's contextually appropriate (e.g., success toast = celebrate; "no results yet" empty state = idle).
- Implementation: the mascot SVG body fill is the only thing that changes vs. other surfaces. A single CSS variable or `currentColor` reference is sufficient.

**Everything else** — module colors, feedback colors, typography, spacing, radii, motion, accessibility — follows `gabee-design-spec.md` unchanged. The admin sees module colors (`--module-numbers` blue, `--module-words` magenta, etc.) when reviewing content for those modules, and that mapping is universal.
